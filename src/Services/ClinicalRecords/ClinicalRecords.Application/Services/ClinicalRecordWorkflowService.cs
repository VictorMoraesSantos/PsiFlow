    ...
    IClinicalAuditLogRepository auditLogs) : IClinicalRecordWorkflowService
{
    ...
    public async Task<Result<object>> CreateRecordAsync(int patientId, int tenantId, string? name, CancellationToken ct) { var existing = await medicalRecords.GetByPatientAndTenantAsync(patientId, tenantId, ct); if (existing is not null) return Result.Success<object>(existing); var r = new MedicalRecord { TenantId = tenantId, PatientId = patientId, Name = string.IsNullOrWhiteSpace(name) ? $"Patient {patientId}" : name }; await medicalRecords.Create(r, ct); return Result.Success<object>(r); }
    ...
    private static Result ValidateEnvelope(string? ciphertext, string? nonce, string? tag) =>
        string.IsNullOrWhiteSpace(ciphertext) || string.IsNullOrWhiteSpace(nonce) || string.IsNullOrWhiteSpace(tag)
            ? Result.Failure(Error.Failure("Encrypted envelope requires ciphertext, nonce and tag."))
            : Result.Success();
}
