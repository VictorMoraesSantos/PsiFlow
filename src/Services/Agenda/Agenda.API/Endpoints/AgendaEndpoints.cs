...
public static class AgendaEndpoints
{
    public static IEndpointRouteBuilder MapAgendaEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapPost("/v1/availability/weekly", async (WeeklyAvailabilityRequest request, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new CreateWeeklyAvailabilityCommand(request, TenantId(user)), ct), value => Results.Created($"/v1/availability/weekly/{value.Id}", value))).RequireAuthorization(Permissions.Agenda.AvailabilityCreate);

        app.MapGet("/v1/availability/weekly", async (ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new GetWeeklyAvailabilitiesQuery(TenantId(user)), ct))).RequireAuthorization(Permissions.Agenda.AvailabilityRead);
        ...
        app.MapPut("/v1/availability/weekly", async (WeeklyAvailabilityReplaceRequest request, ClaimsPrincipal user, IAgendaService service, CancellationToken ct) =>
            ToHttp(await service.ReplaceWeeklyAvailabilitiesAsync(request, TenantId(user), ct))).RequireAuthorization(Permissions.Agenda.AvailabilityUpdate);
        ...
        app.MapPatch("/v1/availability/weekly/{availabilityId:int}", async (int availabilityId, WeeklyAvailabilityRequest request, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new UpdateWeeklyAvailabilityCommand(availabilityId, request, TenantId(user)), ct))).RequireAuthorization(Permissions.Agenda.AvailabilityUpdate);
        ...
        app.MapDelete("/v1/availability/weekly/{availabilityId:int}", async (int availabilityId, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new DeleteWeeklyAvailabilityCommand(availabilityId, TenantId(user)), ct), _ => Results.NoContent())).RequireAuthorization(Permissions.Agenda.AvailabilityDelete);
        ...
        app.MapPost("/v1/schedule-blocks", async (ScheduleBlockRequest request, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new CreateScheduleBlockCommand(request, TenantId(user), UserId(user)), ct), value => Results.Created($"/v1/schedule-blocks/{value.Id}", value))).RequireAuthorization(Permissions.Agenda.BlocksCreate);

        app.MapGet("/v1/schedule-blocks", async (ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new GetScheduleBlocksQuery(TenantId(user)), ct))).RequireAuthorization(Permissions.Agenda.BlocksRead);

        app.MapDelete("/v1/schedule-blocks/{blockId:int}", async (int blockId, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new DeleteScheduleBlockCommand(blockId, TenantId(user)), ct), _ => Results.NoContent())).RequireAuthorization(Permissions.Agenda.BlocksDelete);
        ...
        app.MapGet("/v1/available-slots", async (DateTime from, DateTime to, string? modality, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new GetAvailableSlotsQuery(new AvailableSlotsRequest(from, to, modality), TenantId(user)), ct))).RequireAuthorization(Permissions.Agenda.AvailableSlotsRead);
        ...
        }).RequireAuthorization(Permissions.Agenda.CalendarDayRead);
        ...
        }).RequireAuthorization(Permissions.Agenda.CalendarWeekRead);
        ...
        }).RequireAuthorization(Permissions.Agenda.CalendarMonthRead);
        ...
        app.MapPost("/v1/appointments/{appointmentId:int}/cancel", async (int appointmentId, CancelAppointmentRequest request, ClaimsPrincipal user, ISender sender, CancellationToken ct) =>
            ToHttp(await sender.Send(new CancelAppointmentCommand(appointmentId, request, TenantId(user), UserId(user)), ct))).RequireAuthorization(Permissions.Agenda.AppointmentsCancel);
        ...
        app.MapPost("/v1/appointments/{appointmentId:int}/reschedule", async (int appointmentId, RescheduleAppointmentRequest request, ClaimsPrincipal user, IAgendaService service, CancellationToken ct) =>
            ToHttp(await service.RescheduleAppointmentAsync(appointmentId, request, TenantId(user), UserId(user), ct))).RequireAuthorization(Permissions.Agenda.AppointmentsCreate);
